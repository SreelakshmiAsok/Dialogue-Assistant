// pipeline.js - Whisper STT simulation, RAG vector search, Linguistic Gate, Ontology + LLM evaluation
...
function checkLinguisticConstraints(scenarioId, rawText) {
  ...
  for (const word of DISRESPECT_WORDS) {
    ...
    // Match as word boundary (whole word)
  ...
  for (const verb of BLUNT_VERBS) {
    ...
    if (verbRegex.test(text + " ")) {
      // Check if an honorific suffix is also present → upgraded, allowed
...
window.runFullPipeline = function(scenarioId, userText) {
  ...
  let mascotFeedback   = "";
  let linguisticBlock  = null; // Will carry linguistic gate info if applicable

  // Apply linguistic gate outcomes FIRST (overrides ontology if FAIL/WARNING)
  if (linguisticResult.verdict === "FAIL") {
    ...
    // Don't override main feedback—append the suggestion at the end
  }

  // Score adjustments based on ontology (only if linguistic PASS/WARNING)
  if (linguisticResult.verdict !== "FAIL") {
  ...
  // Generate mascot feedback text (only if linguistic gate didn't already set it)
  if (!mascotFeedback) {
    if (scenarioId === "stranger") {
  ...
  // Append linguistic warning suggestion if present
