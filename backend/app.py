...
def api_auth_assume_child():
    ...
    # Issue a student token for the child
...
def api_evaluate():
    ...
    try:
        validated_data = EvaluateRequest.model_validate(data)
    except ValidationError as e:
        return jsonify({"error": "Validation Error", "details": e.errors()}), 422

    question_id = validated_data.question_id
    response = validated_data.response.strip()
...
def api_progress():
    ...
    if role == "parent":
        ...
        if not student_id:
            return jsonify({"error": "student_id query parameter is required for parents"}), 400
    ...
    progress = get_progress(target_user_id)

    total = len(progress)
    correct = sum(1 for p in progress if str(p.get("correct", "")).lower() == "true")
...
def api_link_child():
    ...
    if not data:
        return jsonify({"error": "No data provided"}), 400
    try:
        validated = LinkChildRequest.model_validate(data)
    except ValidationError as e:
        return jsonify({"error": "Validation Error", "details": e.errors()}), 422
        
    result = link_child_to_parent(request.user["user_id"], validated.child_email)
    if "error" in result:
        return jsonify(result), 400
        
    return jsonify({"message": "Child linked successfully"})
...
def api_admin_system():
    """Diagnostic endpoint for administrators."""
...
def api_tier2_evaluate_turn():
    ...
    try:
        validated_data = Tier2EvaluateTurnRequest.model_validate(data)
    except ValidationError as e:
        return jsonify({"error": "Validation Error", "details": e.errors()}), 422
    ...
    try:
        ...
        pass # don't fail response if progress save fails
